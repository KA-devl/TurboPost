            ...
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="divide-y divide-gray-200">
                ...
                >
                  ...
                  <td class="flex h-px whitespace-nowrap">
                    <a v-if="data.status == 'pending'"
                      class="block"
                      href="javascript:;"
                      data-hs-overlay="#hs-ai-invoice-modal"
                    >
                      <div
                        @click="acquirePackage(data.id, data.status)"
                        class="py-1.5"
                      >
                        <div
...
export default {
  setup() {
    ...
    const acquirePackage = (id, status) => {
      if (status == "delivered" || status == "transit") {
        ...
        try {
          ...
          toast.success("Package acquired successfully");
        } catch (error) {
          toast.error(error.message);
          throw new Error(
            `Failed to acquire package due to error : ${error.message}`
          );
        }
      }
    };
    ...
    onMounted(async () => {
      try {
        ...
        throw new Error(
          `Failed to fetch data due to an api error : ${error.message}`
        );
      }
    });

    return { packageData, formatDate, formatDueDate, count, acquirePackage };
  },
};
</script>
